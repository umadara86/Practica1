...
public class ZoomSeekBar extends View {
    ...
    @Override protected void onDraw(Canvas canvas) {
        ...
        //int y = padding + (altoBar -a - altoregla - altonumero)/2
        ...
        while (v <= escalaMax) {
            if (v >= escalaMin) {
                ...
                if (((v - escalaIni) / escalaRaya) % escalaRayaLarga == 0) {y = yIni + altoBar + altoRegla;
    ...
    @Override public boolean onTouchEvent(MotionEvent event) { int x_0, y_0, x_1, y_1;
        ...
        switch (event.getAction()
            ...
            case MotionEvent.ACTION_UP:
                if(Estado.PALANCA_PULSADA == estado)
                    escuchador.onConectar(getVal());
                estado = Estado.SIN_PULSACION; break;
    ...
    //creando metodos getter y setter para zoomseekbar
